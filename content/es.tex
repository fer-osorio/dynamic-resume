...
\newcommand{\secKeyProjects}{Proyectos Clave}
...
% Subsection titles
% =============================================================================
\newcommand{\subNTRU}{Protocolo de Intercambio de Claves Post-Cuántico [C++, NTRU, Criptografía Reticular]}
\newcommand{\subAES}{Biblioteca de Cifrado AES [C/C++, Estándares NIST, Validación Criptográfica]}
\newcommand{\subPixelLab}{Pixel Lab - Kit de Análisis Criptanalítico de Imágenes [Python, Análisis Estadístico, NumPy]}
...
\newcommand{\subEduTools}{Sitio Web de Fundamentos Matemáticos [HTML, CSS, JavaScript, Criptografía Web]}
...
\newcommand{\roleTitleSecurity}{Ingeniero de Seguridad (Especialización en Criptografía)}
\newcommand{\roleTitleSoftware}{Ingeniero de Software (Especialización en Seguridad)}
...
% =============================================================================
\newcommand{\skillsCryptoSecShared}{\textbf{Criptografía y Seguridad:} Criptografía Post-Cuántica (NTRU, Reticular), Mecanismos de Encapsulamiento de Claves (KEM), RSA, Criptografía de Curva Elíptica (ECC), AES, Modos de Cifrado por Bloques (CBC, CTR, ECB, OFB), Firmas Digitales, Intercambio de Claves, Criptoanálisis, Funciones Hash (SHA-2, SHA-3).}

\newcommand{\skillsProgrammingLangSoftware}{\textbf{Lenguajes de Programación:} C/C++ (Avanzado -- Programación de Sistemas, Optimización de Rendimiento), Python (Scripting, Automatización), Java, Bash Scripting, JavaScript (Desarrollo Web, Visualizaciones Criptográficas), HTML/CSS.}
...
\newcommand{\skillsSoftwareEngSoftware}{\textbf{Ingeniería de Software:} Arquitectura de Sistemas y Patrones de Diseño, Optimización y Perfilado de Rendimiento, Marcos de Pruebas Automatizadas (Unitarias, Integración, Sistema), Pipelines CI/CD, Buenas Prácticas de Revisión de Código, Diseño Modular, Desarrollo de API, Ciclo de Vida del Desarrollo de Software (SDLC).}
...
\newcommand{\skillsDevToolsSoftware}{\textbf{Herramientas y Prácticas de Desarrollo:} Git (Control de Versiones), Make/CMake (Sistemas de Construcción), Docker (Contenedorización), GDB/Valgrind (Depuración y Análisis de Memoria), Herramientas de Perfilado de Rendimiento, Marcos de Pruebas Unitarias, Integración Continua.}
...
\newcommand{\skillsSystemsShared}{\textbf{Sistemas y Plataformas:} Linux, Aplicaciones de Línea de Comandos, Manejo de Formatos de Archivo (Binario, Bitmap, Texto), Gestión de Memoria, Optimización de Bajo Nivel, Programación de Sistemas.}
...
\newcommand{\expOpeningSoftware}{Lideró el desarrollo de software criptográfico para investigación en criptografía post-cuántica, implementando código de calidad productiva en C/C++ con marcos de pruebas exhaustivos y documentación extensa. \textbf{Logró una mejora de rendimiento de 20x en operaciones criptográficas} mediante mejoras algorítmicas y técnicas de optimización de bajo nivel.}
...
\newcommand{\expAesSoftware}{Diseñó y optimizó la biblioteca de cifrado AES \textbf{con almacenamiento de claves HSM basado en PKCS\#11}, logrando \textbf{una mejora de rendimiento de 2x mediante refinamiento algorítmico}, manteniendo conformidad total con NIST FIPS 197.}
...
% SHA shared bullet
\newcommand{\expShaShared}{Implementó el algoritmo hash SHA-512 con soporte para multiplicación de números de 128 bits, \textbf{logrando conformidad total con NIST FIPS 180-4 verificada mediante validación exhaustiva con vectores de prueba}.}
...
\newcommand{\expValidationSoftware}{Construyó marcos de pruebas automatizadas y pipelines CI/CD para la validación de protocolos criptográficos. \textbf{Implementó más de 200 casos de prueba automatizados}, facilitando la integración continua sin regresiones.}
...
\newcommand{\expAnalysisSoftware}{Diseñó una base de código modular y reutilizable con APIs limpias y documentación exhaustiva. \textbf{Redujo el tiempo de integración en un 60\%} mediante interfaces bien definidas y documentación en línea extensa.}
...
\newcommand{\expSeminarsShared}{Presentó resultados de investigación sobre criptografía resistente a ataques cuánticos en \textbf{más de 21 seminarios técnicos}, comunicando eficazmente conceptos complejos de seguridad a audiencias de pregrado y posgrado.}
...
% =============================================================================
\newcommand{\ntruOpeningCrypto}{Implementó un Mecanismo de Encapsulamiento de Claves (KEM) resistente a ataques cuánticos utilizando criptografía reticular NTRU, proporcionando seguridad frente a ataques de computadoras cuánticas.}

\newcommand{\ntruOpeningSecurity}{Implementó un Mecanismo de Encapsulamiento de Claves (KEM) resistente a ataques cuánticos utilizando criptografía reticular NTRU, diseñado para resistir tanto ataques clásicos como cuánticos.}

\newcommand{\ntruOpeningSoftware}{Implementó un Mecanismo de Encapsulamiento de Claves (KEM) resistente a ataques cuánticos utilizando criptografía reticular NTRU con arquitectura C++ limpia y diseño modular.}

\newcommand{\ntruOpeningApplied}{Implementó un Mecanismo de Encapsulamiento de Claves post-cuántico basado en NTRU reticular, traduciendo la criptografía reticular teórica en software validado. \textbf{Demostró la viabilidad práctica de la criptografía post-cuántica con generación de claves en 2ms y cifrado en menos de un milisegundo en hardware de uso general}.}
...
\newcommand{\ntruOptimizationSoftware}{Diseñó algoritmos eficientes de operaciones polinomiales aprovechando las propiedades de la estructura del anillo. \textbf{Redujo la complejidad computacional de $O(n^4)$ a $O(n^2)$}, mejorando el throughput en un 2000\%.}
...
\newcommand{\ntruValidationSoftware}{Construyó una arquitectura modular con subsistemas dedicados para perfilado de rendimiento, pruebas de seguridad y utilidades de depuración. \textbf{Habilitó una iteración de desarrollo 60\% más rápida mediante registro exhaustivo y múltiples formatos de salida} (binario, hexadecimal, ASCII).}
...
\newcommand{\aesOpeningSoftware}{Diseñó y desarrolló una biblioteca de cifrado AES de nivel productivo con conformidad a NIST FIPS 197 y arquitectura modular.}
...
\newcommand{\aesMetricsSoftware}{Desarrolló un manejador de archivos multi-formato compatible con imágenes bitmap, datos binarios y archivos de texto. \textbf{Procesó archivos de hasta 100MB con cero fugas de memoria verificadas mediante Valgrind}.}
...
\newcommand{\pixellabOpeningSoftware}{Desarrolló un paquete Python de nivel productivo para generación de imágenes a nivel de píxel y análisis criptanalítico. \textbf{Publicó una biblioteca instalable mediante pip con más del 80\% de cobertura de pruebas, documentación y herramientas CLI}.}
...
\newcommand{\pixellabStatsSoftware}{Implementó manipulación a nivel de píxel \textbf{con soporte para asignación directa, generación recursiva y procesamiento por lotes}, mostrando desarrollo Python optimizado con integración de NumPy/SciPy.}
...
\newcommand{\pixellabArchitectureSoftware}{Construyó infraestructura de desarrollo completa que incluye pruebas automatizadas, herramientas CLI y scripts de ejemplo. \textbf{Implementó un pipeline CI/CD con pruebas automatizadas en múltiples versiones de Python}.}
...
% =============================================================================
\newcommand{\edutoolsPlatformDesc}{\textbf{Plataforma:} Sitio web público curado que muestra investigación criptográfica, herramientas interactivas y contenido educativo que demuestra conceptos prácticos de criptografía.}

\newcommand{\edutoolsDemoIntro}{\textbf{Demostraciones Criptográficas Interactivas:}}

\newcommand{\edutoolsDemoList}{%
	\item RSA: Generación de claves, flujos de cifrado/descifrado, esquemas de relleno (PKCS\#1, OAEP).
	\item Funciones Hash: Visualización del efecto avalancha, estimación de resistencia a colisiones, cómputo de resúmenes.
	\item Curvas Elípticas: Aritmética de puntos, protocolo de intercambio de claves ECDH, firmas digitales ECDSA.%
}

% =============================================================================
% Professional Skills (soft skills) section — items
% =============================================================================
\newcommand{\softskillOne}{\textbf{Comunicación Técnica:} Presentó más de 21 seminarios técnicos sobre sistemas criptográficos, explicando eficazmente conceptos complejos a audiencias diversas.}
